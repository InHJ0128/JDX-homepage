...
const uploadInlineImage = async (file: File) => {
  ...
  fd.append('image', file); // 서버: upload.single('image')
...
const ActivityList: React.FC = () => {
  ...
  const fetchActivities = async () => {
    setLoading(true);
    try {
      const response = await axios.get<Activity[]>('/api/admin/activities', { withCredentials: true });
  ...
  const handleEdit = async (id: number) => {
    const { data: row } = await axios.get<ActivityDetail>(`/api/admin/activities/${id}`, { withCredentials: true });
    setForm({
      id: row.id,
      title: row.title ?? '',
  ...
  // drop/paste로 들어온 이미지 업로드 → 본문에 삽입
  ...
  const toggleHidden = async (id: number, hidden: boolean) => {
    try {
      await axios.patch(`/api/admin/activities/${id}/hide`, { hidden: !hidden }, { withCredentials: true });
      setActivities(prev => prev.map(a => a.id === id ? { ...a, hidden: !hidden } : a));
    } catch (err: any) {
      console.error(err);
  ...
  const toggleEmphasized = async (id: number, emphasized: boolean) => {
    try {
      await axios.patch(`/api/admin/activities/${id}`, { emphasized: !emphasized }, { withCredentials: true });
      setActivities(prev => prev.map(a => a.id === id ? { ...a, emphasized: !emphasized } : a));
    } catch (err: any) {
      console.error(err);
