  ...
  useEffect(() => {
    if (!id) {
      fetch(`/api/user/activities?lang=${language}`)
        .then(res => res.json())
        .then(setActivities)
        .catch(err => console.error("활동 목록 불러오기 실패:", err));
    }
  }, [id, language]);
  ...
  useEffect(() => {
    if (id) {
      fetch(`/api/user/activities/${id}?lang=${language}`)
        .then(res => res.json())
        .then(setActivity)
        .catch(err => console.error("활동 상세 불러오기 실패:", err));
    }
  }, [id, language]);
  ...
  if (id && activity) {
    return (
      <div className="max-w-3xl mx-auto p-6">
        ...
        />
        <Link to="/activity" className="text-blue-600 hover:underline block mt-6">
